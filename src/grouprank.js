...
// Resolve a Roblox username-or-ID string (same convention as /forceverify).
async function resolveRoblox(input) {
  const trimmed = input.trim();
  if (/^\d+$/.test(trimmed)) return getUserInfo(trimmed);
  const found = await getUserIdFromUsername(trimmed);
...
export async function groupAcceptCommand(interaction) {
  ...
  const info = await resolveRoblox(robloxInput);
  ...
  await interaction.editReply(
    `✅ Accepted **${info.name}** (ID \`${info.id}\`) into group \`${gcfg.group_id}\`.`,
  );

  const logEmbed = baseEmbed('✅ Group Join Request Accepted')
    .setDescription(`**${info.name}** (ID \`${info.id}\`) was accepted into group \`${gcfg.group_id}\`.`)
    .addFields(
...
export async function groupKickCommand(interaction) {
  ...
  await interaction.editReply(
    `✅ Kicked **${info.name}** (ID \`${info.id}\`) from group \`${gcfg.group_id}\`.`,
  );

  const logEmbed = baseEmbed('👢 Member Kicked')
    .setDescription(`**${info.name}** (ID \`${info.id}\`) was kicked from group \`${gcfg.group_id}\`.`)
    .addFields(
      { name: 'Roblox ID', value: `\`${info.id}\``, inline: true },
...
export async function groupUnkickCommand(interaction) {
  ...
  await interaction.editReply(
    `✅ **${info.name}** (ID \`${info.id}\`) is no longer kicked from group \`${gcfg.group_id}\` — they can rejoin.`,
  );

  const logEmbed = baseEmbed('♻️ Member Un-kicked')
    .setDescription(`**${info.name}** (ID \`${info.id}\`) was un-kicked from group \`${gcfg.group_id}\` and may rejoin.`)
    .addFields(
      { name: 'Roblox ID', value: `\`${info.id}\``, inline: true },
