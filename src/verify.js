...
import { hasRole, isAdmin, baseEmbed, deny, syncNickname, nicknameFailureNote } from './utils.js';
...
export async function forceVerifyCommand(interaction) {
  ...
  try {
    ...
    if (!nickResult.ok) nickNote = `\n⚠️ ${nicknameFailureNote(nickResult.reason)}`;
  } catch {
    // Not resolvable as a guild member (e.g. left the server) — skip the nickname.
  }

  await interaction.editReply(
    `✅ Manually verified ${target} as **${info.name}** (ID \`${info.id}\`).${nickNote}`,
  );
}
...
export async function handleVerifyComponent(interaction) {
  ...
  if (id.startsWith('verify:confirm:')) {
    ...
    const nickNote = nickResult.ok ? '' : `\n⚠️ ${nicknameFailureNote(nickResult.reason)}`;
    return interaction.editReply(
      `✅ Verified! You’re linked to **${info.name}** (ID \`${info.id}\`).${nickNote}`,
    );
  }
}
